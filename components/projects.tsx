        ...
        {featured && (
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-3">
              <span className="text-9xl font-black text-white whitespace-nowrap">{featured.title}</span>
            </div>
                ...
                <div className="flex gap-6 pt-2">
                  {featured.liveDemoUrl && featured.liveDemoUrl !== 'PLACEHOLDER' && (
                    <a 
                      ...
                      className="flex items-center gap-1 text-foreground hover:text-white text-sm transition-colors"
                    >
          ...
          {categories.map((cat) => (
            <motion.button
              ...
              className={`px-4 py-2 rounded-full font-mono text-xs uppercase tracking-wider transition-all border ${
                activeFilter === cat
                  ? 'border-foreground text-white bg-foreground/5'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((project, idx) => (
            ...
            >
              ...
              {/* Title and subtitle */}
              <div className="mb-4">
                <h3 className="text-lg font-bold font-heading text-foreground group-hover:text-white transition-colors mb-1">
                  {project.title}
                </h3>
              ...
              <div className="flex gap-4 pt-4 border-t border-border/50">
                {project.liveDemoUrl && project.liveDemoUrl !== 'PLACEHOLDER' && (
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 text-foreground hover:text-white hover:underline text-xs font-medium transition-colors"
                  >
